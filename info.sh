#!/bin/bash
info () {
bundleDu="$(du -h /root/deploy/bundle/koffer-bundle.collector-apps.tar | awk '{print $1}' | head -n 1)"
bundlePath="$(ls /root/deploy/bundle/koffer-bundle.collector-apps.tar | head -n 1 | sed 's/root\/deploy/tmp\/platform/')"
cat <<EOF
